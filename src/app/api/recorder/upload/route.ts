...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const relay of allRelays) {
      ...
      // Parser distributes splits to legs correctly (ceil-based equal-group).
      // Just pass through the parser's per-leg splits directly.
      if (relay.legs?.length > 0) {
        ;(match as any)._parsedLegs = relay.legs.map((leg: any, idx: number) => ({
          leg: idx + 1,
          splits: leg.splits || [],
        }))
      }
